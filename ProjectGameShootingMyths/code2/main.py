...
class Game:
    def __init__(self):
        ...
        # Menu assets
        self.menu_bg = pygame.image.load(join('images', 'menu', 'menubg.png')).convert_alpha()
        self.play_btn_img = pygame.image.load(join('images', 'menu', 'playButton.png')).convert_alpha()
        self.music_btn_img = pygame.image.load(join('images', 'menu', 'musiconoffButton.png')).convert_alpha()
        self.quit_btn_img = pygame.image.load(join('images', 'menu', 'quitButton.png')).convert_alpha()

        # Scale all buttons proportionally
        button_scale = 0.5  # Adjust this (0.5 = 50% size, 1 = original size)
        self.play_btn_img = pygame.transform.scale(
            self.play_btn_img,
            (int(self.play_btn_img.get_width() * button_scale), int(self.play_btn_img.get_height() * button_scale))
        )
        self.music_btn_img = pygame.transform.scale(
            self.music_btn_img,
            (int(self.music_btn_img.get_width() * button_scale), int(self.music_btn_img.get_height() * button_scale))
        )
        self.quit_btn_img = pygame.transform.scale(
            self.quit_btn_img,
            (int(self.quit_btn_img.get_width() * button_scale), int(self.quit_btn_img.get_height() * button_scale))
        )
        # Setup
        self.setup()
    ...
    def draw_hud(self):
        ...
        # Ammo (bottom-right)
        ammo_text = self.font.render(f'Ammo: {self.ammo} / {self.max_ammo}' + (' (Reloading...)' if self.is_reloading else ''), True, (
108,70,31))
    ...
    def draw_menu(self):
        ...
        # Button positions
        spacing = 20
        start_rect = self.play_btn_img.get_rect(center=(WINDOW_WIDTH // 2, 350))
        music_rect = self.music_btn_img.get_rect(center=(WINDOW_WIDTH // 2, 350 + self.play_btn_img.get_height() + spacing))
        exit_rect = self.quit_btn_img.get_rect(center=(WINDOW_WIDTH // 2, 350 + (self.play_btn_img.get_height() + spacing) * 2))

        # Draw buttons
        self.display_surface.blit(self.play_btn_img, start_rect)
        self.display_surface.blit(self.music_btn_img, music_rect)
        self.display_surface.blit(self.quit_btn_img, exit_rect)

        # Optional toggle label
        if not self.music_playing:
            music_txt = self.font.render("", True, (255, 0, 0))
            self.display_surface.blit(music_txt, (music_rect.centerx + 120, music_rect.centery - music_txt.get_height() // 2))
